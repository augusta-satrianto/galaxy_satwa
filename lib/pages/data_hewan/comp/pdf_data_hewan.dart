...
class PdfDataHewan {
  ...
  static Future<File> generate(List<dynamic> items) async {
    ...
    final tableHeaders = ['Gambar', 'Nama', 'Kategori', 'Jenis', 'Umur'];
    List<Uint8List> gambar = [];
    // Fetch the network image asynchronously
    for (int i = 0; i < items.length; i++) {
      PetModel pet = items[i];
      final Uint8List netImageBytes = await _fetchNetworkImage(pet.image!);
                ...
                ...items.map((item) {
                  ...
                  return pw.TableRow(
                    children: [
                      ...
                      // Text cells
                      ...[item.name, item.category, item.type, item.old]
                          .map((text) {
                        return pw.Container(
                          alignment: pw.Alignment.topLeft,
                          margin: const pw.EdgeInsets.all(0),
    ...
    return FileHandleApi.saveDocument(
      name: 'Data Hewan_$randomNumber.pdf',
      pdf: pdf,
    );
  }
}
